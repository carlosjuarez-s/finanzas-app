...
async function crearDevolucion(b: Record<string, unknown>) {
  ...
  const existe = await db.query.prestamosPersonales.findFirst({
    where: eq(prestamosPersonales.id, prestamoId), columns: { id: true },
  });
...
export async function POST(req: NextRequest) {
  ...
  try {
    if (b?.entidad === 'devolucion') {
      const r = await crearDevolucion(b);
...
export async function PATCH(req: NextRequest) {
  ...
  try {
    const [fila] = await db.update(prestamosPersonales).set({
      ...
      notas: texto(b.notas),
    }).where(eq(prestamosPersonales.id, String(b.id)))
      .returning({ id: prestamosPersonales.id });
...
export async function DELETE(req: NextRequest) {
  ...
  try {
    ...
    else await db.delete(prestamosPersonales).where(eq(prestamosPersonales.id, id));
    return NextResponse.json({ ok: true });
  } catch (e) {
    return NextResponse.json({ error: mensajeDeError(e) }, { status: 500 });
  }
